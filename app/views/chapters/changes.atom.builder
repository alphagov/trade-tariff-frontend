atom_feed do |feed|
  feed.title("Trade Tariff - changes for #{@chapter.goods_nomenclature_item_id}: #{@chapter}")
  ...
  @changes.each do |change|
    feed.entry(
      ...
      url: chapter_url(id: @chapter.goods_nomenclature_item_id, anchor: change.anchor_link)
    ) do |entry|
